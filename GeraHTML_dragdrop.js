...
function DROP(d_ev)
{
          ...
          if (objeto.tipoobj==10 || objeto.tipoobj==23 || objeto.tipoobj==8) {
	        ...
	        if (objeto.tipoobj==10) 
              ...
              else
              {
                if (objeto.tipoobj==23) 
                {
				  //var str = "<img src='" + objeto.imagem + "' width='" + obj_soltar.width + "' height='" + obj_soltar.height + "'>";
                  //dd.obj.write(str);
				  var i=1,objArr,id, sai = false;
				  try {
					while(sai == false) {
						id = "DArrastarImgN"+ String(i) + obj_soltar.id.substring((obj_soltar.id.length - 1), obj_soltar.id.length);
						objArr = document.getElementById(id);
						if (objArr == null)
							sai = true;
						else {
							if (objArr.id == objeto.id) {
								objeto.feito = 1;
								document.getElementById(id.substring(1)).style.borderStyle = "none";
							} else {
								if (objArr.feito == 1) {
									objArr.style.top = "-"+ String((objArr.height + 10000)) +"px";
									objArr.style.left = "-"+ String(objArr.width) +"px";
								}
							}
						}
						i++;
				  ...
				  document.getElementById(objeto.id.substring(1, objeto.id.length)).src=objeto.imagem;
                  dd.obj.moveTo(obj_soltar.x_ini, obj_soltar.y_ini);
				  dd.obj.visible=false;
				  document.getElementById(objeto.id.substring(1, objeto.id.length)).style.width = String(obj_soltar.width) +"px";
				  document.getElementById(objeto.id.substring(1, objeto.id.length)).style.height = String(obj_soltar.height) +"px";
				  obj_soltar.style.top = "-"+ String(obj_soltar.height) +"px";
				  obj_soltar.style.left = "-"+ String(obj_soltar.width) +"px";
                  try {
                  if (obj_soltar.som!='') {
                    if (somswf==0) {
                      toca_somNovo(obj_soltar.som, false);
 ...
 //                 dd.obj.hide();
            ...
            else
            {
			   var ret = null;
			   if (objeto.tipoobj == 8) {
			      ret = retIndiceSoltado(objeto);
				   /*if (objeto.tipoobj != 8 || FindObj('DSoltar' + objeto.indice_soltar).errado == 1) {
					  objeto.atual = 1; // acerto
					  arrastando=false; 
					  obj_soltar = FindObj('DSoltar' + objeto.indice_soltar);
					  obj_soltar.feito = 1;
					  obj_soltar.atual = 1;
					  dd.obj.write('<img src="' + obj_soltar.img + '" width="' + obj_soltar.width + '" height="' + obj_soltar.height + '">');
					  ...
					  if ((colou == false && erradoArrastou == false) || ret == null || (ret != objeto.indice_soltar && FindObj('DSoltar' + objeto.indice_soltar).errado == 0)) {//não acertou nem errou
						  dd.obj.moveTo(objeto.x_ini, objeto.y_ini);
						  arrastando = false;
					  } else {
						  obj_soltar = FindObj('DSoltar' + ret);
						  if (obj_soltar.feito == 1) {
							 if (arrastando) {
							 ...
							 obj_soltar.feito = 1;
							 var ob = FindObj('DSoltar' + objeto.indice_soltar);
							 dd.obj.write('<img src="' + ob.img + '" width="' + obj_soltar.width + '" height="' + obj_soltar.height + '">');
							 dd.obj.moveTo(obj_soltar.x_ini, obj_soltar.y_ini);
							 dd.obj.visible=false;
							 objeto.obj = dd.obj;
							 arrastando=false;
							 obj_soltar.indice_errado = objeto.indice_soltar;
						  }
					  }
				   //}
				} else {
...
function dragCorreto(d_ev, objeto)
{
    var volta = false;
	erradoArrastou = false;
    var obj_soltar;
...
function retIndiceSoltado(objeto) {
    var obj_soltar;
    for (i=1; i<= n_Arrastar; i++) {
      obj_soltar = FindObj('DSoltar' + i);
      if ((dd.obj.x>=obj_soltar.x_ini
